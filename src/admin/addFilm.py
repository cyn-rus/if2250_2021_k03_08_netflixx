...
class addFilm(tk.Frame):
    def __init__(self, user, master=None):
        ...
        self.master = master
        self.pack()
        self.add_film_page(user)

    def add_film_page(self, user):
        ...
        self.submit = tk.Button(text="Add Film", font=buttonFont, bg="#010109", fg="#9f64d8", activebackground="#010109", activeforeground="#9f64d8", command=lambda: self.isAddFilmValid(user))
        self.submit.place(x=590, y=650, width=140, height=40)

    def isAddFilmValid(self, user):
        ...
        template.writeFile([tanggal, "add"+" "+id], user+".csv")

        template.changePage(self, "add film", user)
